...
# 
# Converts a single csv file into a single or multiple android string xml files.
# Creates a directory for each file with the android specific naming convention.
...
module CSV2Android
  class Converter
    ...
    def file_path_for_locale(foldername)
      require 'pathname' 
      Pathname.new(self.default_path) + "values-#{foldername}" + "strings.xml"
    end
    ...
    # Convert csv file to multiple android xml files for each column
    def convert(name = self.csv_filename)
      ...
      CSVParserClass.foreach(name, :quote_char => '"', :col_sep =>',', :row_sep => :auto) do |row|
        ...
        # run through columns
        row.size.times do |i|
          # ignore the column if it is definied in excludedCols in settings file
